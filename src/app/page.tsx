...
// import Animations from "./components/page";
import Contact from "./components/sendmail";
import LatestWork from "./components/latesWork";
import Fotter from "./components/fotter";
import dynamic from "next/dynamic";

// Disable SSR for components that cause hydration issues
const AnimationsWithNoSSR = dynamic(() => import("./components/page"), { ssr: false });
// const ModalWithNoSSR = dynamic(() => import("./components/Modal"), { ssr: false });
...
export default function Home() {
  ...
  // Prevent server-side rendering mismatch by rendering only client-side content
  if (!isClient) {
    return null;
  }
  ...
  return (
    <body>
      <section className="flex flex-col justify-between h-screen">
        <AnimationsWithNoSSR />
        <header className="flex mx-auto mt-0 w-full justify-between sm:max-w-full md:max-w-6xl lg:max-w-screen-xl relative px-4 sm:px-6 md:px-8">
          {/* SVG Picture */}
          <svg
            version="1.1"
          ...
          <div className="flex items-center text-[#b0b2c3] space-x-6">
            {/* Phone Icon */}
            ...
            {/* Whatsapp Icon */}
            <a href="https://wa.me/+923408062138?text=Only Chat" target="_blank">
              <svg
                aria-hidden="false"
                focusable="false"
                data-prefix="fab"
          ...
          <div className="relative self-center after:contents-[' '] after:absolute after:w-[2px] after:h-5 after:bg-white after:left-1/2 after:-translate-x-1/2 after:top-[75px]">
            <button
              onClick={() => {
                const section = document.getElementById("latest-works");
                section?.scrollIntoView({ behavior: "smooth" });
              }}
              className="bg-[#4595eb] mb-14 py-2 px-5 duration-300  rounded font-extrabold bg-gradient-to-l from-[#1595b6] to-[#1f2667e6] hover:scale-105 ease-in-out relative group "
            >
              latest Work
              <svg
                version="1.1"
